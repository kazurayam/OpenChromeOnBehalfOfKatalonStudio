...
As ["google chrome crashed on my pc ---- 2 reasons found"]( https://forum.katalon.com/discussion/6150/google-chrome-crashed-on-my-pc-----2-reasons-found) describes, Katalon Studio on my PC always fails to open Google Chrome browser. There reason I found out was as follows.
...
By looking at the log file of chromedriver.exe, I realized that the Katalon Studio tells chromedriver to generate a command to start chrome.exe with `--disable-extensions` switch. This switch causes problem with ForceInstalledExtensions. So I want to start the Chrome without `--disable-extension` switch. By looking around the Katalon Forum I got an idea:

1. I will not rely on Katalon Studio for opening browser.
...
```shell:TC1_defaultWayOfOpeningBrowser
